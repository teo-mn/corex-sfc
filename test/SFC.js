...
async function sealEpoch(sfc, duration, _validatorsMetrics = undefined) {
    ...
    if (validatorsMetrics === undefined) {
        validatorsMetrics = {};
        for (let i = 0; i < validatorIDs; i += 1) {
            validatorsMetrics[i] = {
    ...
    describe('Basic functions', () => {
        describe('Constants', () => {
            ...
            it('Returns minimum amount to stake for a Validator', async () => {
                expect((await this.sfc.minSelfStake()).toString()).to.equals('200000000000000000');
            });
            ...
            it('Returns the ratio of the reward rate at base rate (without lockup)', async () => {
                expect((await this.sfc.unlockedRewardRatio()).toString()).to.equals('500000000000000000');
            });
            ...
            it('Returns the version of the current implementation', async () => {
                expect((await this.sfc.version()).toString()).to.equals('0x313031');
            });
...
contract('SFC', async ([firstValidator, secondValidator, thirdValidator, firstDelegator, secondDelegator, thirdDelegator]) => {
    ...
    describe('Methods tests', async () => {
        ...
        it('checking createValidator function', async () => {
            ...
            expect(await this.sfc.totalStake.call()).to.be.bignumber.equal(amount18('0.7'));
            ...
            expect(firstValidatorObj.receivedStake).to.be.bignumber.equal(amount18('0.2'));
            ...
            expect(await this.sfc.getStake.call(firstValidator, firstValidatorID)).to.be.bignumber.equal(amount18('0.2'));
            ...
            expect(this.node.nextValidators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('0.2'));
        ...
        it('checking sealing epoch', async () => {
            ...
            expect(this.node.validators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('0.2'));
            expect(this.node.validators[secondValidatorID.toString()]).to.be.bignumber.equal(amount18('0.8'));
            expect(Object.keys(this.node.nextValidators).length).to.equal(3);
            expect(this.node.nextValidators[firstValidatorID.toString()]).to.be.bignumber.equal(amount18('0.3'));
            expect(this.node.nextValidators[secondValidatorID.toString()]).to.be.bignumber.equal(amount18('0.8'));
...
contract('SFC', async ([firstValidator, secondValidator, thirdValidator, testValidator, firstDelegator, secondDelegator, account1, account2, account3, account4]) => {
    ...
    describe('Staking / Sealed Epoch functions', () => {
        ...
        it('Check pending Rewards of delegators', async () => {
            ...
            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals('11610');
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals('4590');
        });
        ...
        it('Check if pending Rewards have been increased after sealing Epoch', async () => {
            ...
            await sealEpoch(this.sfc, (new BN(60 * 60 * 24)).toString());
            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals('23220');
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals('9180');
        });
        ...
        it('Should increase balances after claiming Rewards', async () => {
            ...
            expect(firstDelegatorPendingRewards).to.be.bignumber.equal(amount18('0.459'));
            ...
            expect(firstDelegatorBalance.add(firstDelegatorPendingRewards)).to.be.bignumber.below(delegatorBalance.add(amount18('0.02')));
        });
        ...
        it('Should increase stake after restaking Rewards', async () => {
            ...
            const firstDelegatorPendingRewards = await this.sfc.pendingRewards(firstDelegator, firstValidatorID);
             expect(firstDelegatorPendingRewards).to.be.bignumber.equal(new BN('4590'));
        ...
        it('Should increase locked stake after restaking Rewards', async () => {
            ...
            const firstDelegatorPendingRewards = await this.sfc.pendingRewards(firstDelegator, firstValidatorID);
            expect(firstDelegatorPendingRewards).to.be.bignumber.equal(new BN('5967'));
            const firstDelegatorPendingLockupRewards = new BN('3672');
            const firstDelegatorStake = await this.sfc.getStake(firstDelegator, firstValidatorID);
            const firstDelegatorLockupInfo = await this.sfc.getLockupInfo(firstDelegator, firstValidatorID);
        ...
        it('Should return stashed Rewards', async () => {
            ...
            expect((await this.sfc.rewardsStash(firstDelegator, 1)).toString()).to.equals('4590');
        });
    ...
    describe('Stake lockup', () => {
        ...
        it('Check pending Rewards of delegators', async () => {
            ...
            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals('16834');
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals('4819');
        });
        ...
        it('Check if pending Rewards have been increased after sealing Epoch', async () => {
            ...
            await sealEpoch(this.sfc, (new BN(60 * 60 * 24)).toString());
            expect((await this.sfc.pendingRewards(firstValidator, firstValidatorID)).toString()).to.equals('33668');
            expect((await this.sfc.pendingRewards(firstDelegator, firstValidatorID)).toString()).to.equals('9639');
        });
        ...
        it('Should return stashed Rewards', async () => {
            ...
            await this.sfc.stashRewards(firstDelegator, 1);
            expect((await this.sfc.rewardsStash(firstDelegator, 1)).toString()).to.equals('4819');
        });
        ...
        it('Should return pending rewards after unlocking and re-locking', async () => {
            ...
            for (let i = 0; i < 2; i += 1) {
                ...
                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(new BN(351823));
                expect(await this.sfc.pendingRewards(secondDelegator, 2)).to.be.bignumber.equal(new BN(335070));
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(new BN(1228918));
                expect(await this.sfc.pendingRewards(secondValidator, 2)).to.be.bignumber.equal(new BN(847530));
                ...
                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(new BN(351823 + 4590));
                expect(await this.sfc.pendingRewards(secondDelegator, 2)).to.be.bignumber.equal(new BN(335070 + 4590));
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(new BN(1228918 + 16834));
                expect(await this.sfc.pendingRewards(secondValidator, 2)).to.be.bignumber.equal(new BN(847530 + 11610));
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                ...
                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(new BN(351823 + 4590 + 665550));
                expect(await this.sfc.pendingRewards(secondDelegator, 2)).to.be.bignumber.equal(new BN(335070 + 4590 + 665550));
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(new BN(1228918 + 16834 + 2441002));
                expect(await this.sfc.pendingRewards(secondValidator, 2)).to.be.bignumber.equal(new BN(847530 + 11610 + 1683450));
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                ...
                expect(await this.sfc.pendingRewards(firstDelegator, 1)).to.be.bignumber.equal(new BN(351823 + 4590 + 665550 + 4590));
                expect(await this.sfc.pendingRewards(secondDelegator, 2)).to.be.bignumber.equal(new BN(335070 + 4590 + 665550 + 4590));
                expect(await this.sfc.pendingRewards(firstValidator, 1)).to.be.bignumber.equal(new BN(1228918 + 16834 + 2441002 + 11610));
                expect(await this.sfc.pendingRewards(secondValidator, 2)).to.be.bignumber.equal(new BN(847530 + 11610 + 1683450 + 11610));
                expect(await this.sfc.highestLockupEpoch(firstDelegator, 1)).to.be.bignumber.equal(epoch.add(new BN(1)));
                expect(await this.sfc.highestLockupEpoch(firstValidator, 1)).to.be.bignumber.equal(epoch.add(new BN(3)));
...
contract('SFC', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
    ...
    describe('Test Rewards Calculation', () => {
        it('Calculation of validators rewards should be equal to 30%', async () => {
            ...
            expect(parseInt(rewardAcc1, 10) + parseInt(rewardAcc2, 10) + parseInt(rewardAcc3, 10)).to.equal(31250 + 15625 + 6241);
        });
...
contract('SFC', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
    ...
    describe('Test Calculation Rewards with Lockup', () => {
        ...
        it('Should not unlock if not locked up CRX', async () => {
            await sealEpoch(this.sfc, (new BN(1000)).toString());
        ...
        it('Should unlock after period ended and stash rewards', async () => {
            ...
            expect(unlockedStake.toString()).to.equal('9000000000000000000');
            expect(web3.utils.fromWei(pendingRewards.toString(), 'ether')).to.equal('29321.125778331257777946');
            ...
            unlockedStake = await this.sfc.getUnlockedStake(thirdDelegator, testValidator3ID, { from: thirdDelegator });
            expect(unlockedStake.toString()).to.equal('10000000000000000000');
            expect(web3.utils.fromWei(pendingRewards.toString(), 'ether')).to.equal('227044.202701408180854866');
